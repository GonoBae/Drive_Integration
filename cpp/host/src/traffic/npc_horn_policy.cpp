    ...
    double dt_seconds, const NpcHornObservation& observation) noexcept
{
    ...
    if (!observation.enabled || observation.nonresponsive_obstacle_id != 0
        || observation.motion_context == NpcHornMotionContext::Inactive) {
        // A crash victim cannot respond to a reminder or TTC warning. Cancel
        // even an ongoing pulse immediately; only uninjured traffic receives
        // preventive warnings. Lifecycle pause and crashed drivers are silent.
