...
| 작성일 | 2026-08-19 |
| 최종 수정 | 2026-09-17 |
| R1 | 수동운전 버티컬 슬라이스; 이전 Core 9/7·확장 9/11 목표는 미인수. 현재 안정화→실측→배포 인수 순서 |
...
**9/17 기준:** 59 lanes·신호 head 16개·교차로 2곳, NPC10·보행자8과 목적지 선택·우회·근접
...
9/9 당시 CTest **46/46**, Unreal **114/114**, 후속 중앙선 검사 **9/9**를 통과했다.
최신 변경의 시험 종류·실행 버전·결과는 [9/17 작업 기록](./worklogs/2026-09-17.md)에 구분한다.
...
9/7 Core 목표·9/8 수정 버퍼, 9/11 확장·9/14 위험 버퍼는 지난 계획이며 현재 완료일로 표시하지 않는다.
RGB smoke는 Should, LiDAR payload는 Future다. 오토바이는 네 접점 축약 모델로 실제 이륜
균형 물리가 아니다. 플레이어와 NPC 모두 세단·경차·트럭의 축약 실내·운전자와 오토바이
탑승자를 표시한다. 공통 부품을 배치한 모델이며 차종별 정밀 실내나 표시 품질 인수와는 다르다.
...
| MAP-005 | Must | 구현 중 | LaneGraph 자동 생성 | `virtual_city_v1` 25개 차선을 보존하고 `signal_city_v2` 59개 lane·두 교차로·좌/직/우 차로와 같은 방향 차선 변경을 저작 원본에서 생성한다. `lane_changes`의 대상·station·대응·실제 충돌면/차선폭을 검증한다. 북부 루프와 보호 좌회전까지 반영한 현재 collision `2232cb521030ae4f`/traffic `f74f3fc490a002f2`를 사용한다. 최종 주행·시각 정렬 인수는 후속; GIS import는 요구하지 않음 | NPC와 경로계획의 공통 그래프 |
