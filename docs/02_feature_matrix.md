...
| PHY-002 | Must | 실험 | 차량 물리 SDK 어댑터 | PhysX Vehicle2 또는 Chrono::Vehicle이 `IVehicleDynamics` 뒤에서 동작 | SDK 교체와 비교 시험 가능 |
| PHY-003 | Must | 결정 | 로컬 ENU 물리 좌표 | 위·경도를 직접 적분하지 않고 meter 단위 ENU에서 계산 | GNSS 변환과 대규모 월드 대응 |
| PHY-004 | Must | 결정 | 고정 시뮬레이션 tick | 렌더 FPS 변화와 무관하게 기본 60Hz로 진행; substep 설정 가능 | headless·재생·학습에 재사용 |
| PHY-005 | Must | 결정 | 기본 차량 동역학 | 질량, 관성, 휠베이스, 조향, 제동, 구동계, 타이어, 서스펜션 포함 | 차량 JSON 교체로 다른 차종 지원 |
...
| PHY-009 | Must | 결정 | 입력 안전장치 | 입력 범위 clamp, 250ms 기본 timeout, 연결 중단 시 감속·정지 | 자율주행 fail-safe |
| PHY-010 | Must | 결정 | 물리 파라미터 파일화 | 차량 수치가 코드가 아닌 버전 관리 설정 파일에 존재 | 차량 교체·튜닝·시험 |
| PHY-011 | Must | 결정 | 물리 회귀 시험 | 직진, 정지, 회전, 경사, 충돌, replay 시험 자동 실행 | SDK 업데이트 검증 |
...
| NET-002 | Must | 제안 | 단일 공통 Protobuf 스키마 | C++·Unreal·Python 생성물이 하나의 원본 Proto에서 생성됨 | 스키마 중복 제거 |
...
| DEC-F02 | 물리 SDK: PhysX Vehicle2 또는 Chrono::Vehicle | D1 | WP-02 이후 시작 불가 |
