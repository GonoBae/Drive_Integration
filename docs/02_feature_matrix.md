...
8/31 최신 후속: 회전좌표 적분의 가짜 에너지 증가를 제거했지만 최고속 full-lock yaw 반전은
아직 미해결이다. 별도로 NPC 1대의 주행 기반을 선행했다.
[NPC 구현·검증 경계](./npc_lane_following.md), [최신 자동 검증](./worklogs/2026-08-31.md)을 따른다.
...
9/2 최신 자동 상태는 keyboard 미세 조향·rear side-brake drift·F3 overlay·full-body
...
|---|---|---|---|---|---|
| MAP-001 | Must | 구현 중 | 가상 도심 MapPackage | Unreal-owned configurable height/normal/material snapshot(일반 기본100cm, 가상 도심50cm), v1/v2 호환, 전체-bounds preflight/Fit·2M sample 상한, marker authoring·manifest-last·strict loader·250ms 감시/tick swap 구현. 실제 `virtual_city_v1`은 Ground 390개, `SIMGHF2` 401×481/192,881 samples 중 192,079 valid samples와 191,200 valid·drivable cells, static OBB 228개이며 checksum `e1805b…`, 저장 맵 재로드·C++ 실제 package 검증을 통과. 별도 `signal_city_v2`는 227 boxes/Ground124/static49/401×481@50cm와 checksum `7446108a…`로 생성·exact validation을 통과했다. 기존 Landscape `SIMGHF1`/0 markers는 보존. 가상 코스 PIE·변경 재Bake/reconnect, versioned 로컬 원점·spawn metadata는 후속 | 다른 도시·트랙도 같은 포맷 사용 |
...
| MAP-005 | Must | 구현 중 | LaneGraph 자동 생성 | `virtual_city_v1`의 25개 방향 차선/T자 연결과 별도 `signal_city_v2`의 42개 lane·두 교차로 movement/비정형 loop를 저작 원본에서 생성하고 topology·실제 충돌면/차선폭을 검증. NPC 경로 추종·Editor override와 최종 주행 인수는 후속; GIS import는 요구하지 않음 | NPC와 경로계획의 공통 그래프 |
...
| PHY-007 | Must | 구현 중 | 동적 충돌 프록시 | NPC OBB·보행자 vertical capsule에 opt-in 질량·yaw inertia·속도 한도를 두고 Ego와 질량비에 따른 equal-and-opposite normal/tangent impulse를 계산한다. 질량 0은 기존 무한질량 호환 경로이며 결정성·reset·실제 host 반작용 시험은 통과했다. NPC↔NPC, ragdoll과 실제 PIE 충돌 감각은 후속 | 향후 다중 에이전트 |
...
| PHY-010 | Must | 구현 | 물리 파라미터 파일화 | strict `vehicle_sedan.cfg`를 v7로 전환해 `front_tire_corner_stiffness_n_rad`와 `rear_tire_corner_stiffness_n_rad`를 타이어 1개당 값으로 분리한다. v6는 구 `tire_corner_stiffness_n_rad`를 두 새 항목에 각각 복사한 뒤 구 항목을 삭제하고 version7로 올려야 하며 v5는 `comfortable_lateral_accel_mps2`도 삭제한다. 구 버전은 migration 오류, v7의 legacy key는 unknown 오류로 거부하고 자동 변환하지 않는다. v6 검증은 이력이며 v7 config 회귀와 최종 CTest 16/16 통과 | 차량 교체·튜닝·시험 |
...
| ID | 우선순위 | 상태 | 기능 | R1 완료 기준 | 향후 재사용 |
|---|---|---|---|---|---|
| UE-001 | Must | 구현 | 수동 입력 경로(현재 Pawn 내장) | W/S/A/D·Space와 게임패드 입력에 deadzone을 적용한다. 키보드 A/D는 `DeltaSeconds` 기반 rise/return으로 frame-independent하며 최신 변화값 30Hz·유지 명령 20Hz heartbeat를 사용한다 | 향후 ManualInputComponent로 분리해 AI 명령과 같은 포맷 사용 |
| UE-002 | Must | 구현 | ExternalVehiclePawn | C++ Ego pose·4개 wheel state를 표시하되 시각 휠 회전은 부호 있는 차체 종방향 속도와 타이어 반지름에 연동하고 정지 시 0으로 고정한다. 자체 세단/원형 휠·10종 material·unit root·독립 pivot·NoCollision과 실제 렌더 검증을 유지. 접지 상실 시 이전 조향각이 고정되던 버그를 수정해 지지 위치는 보존하고 최신 서버 조향각은 body-up 기준으로 갱신한다. 실제 Pawn Tick fixture 재현→수정·UE 26/26 통과는 표시 증거이며 서버 주행 증거와 구분. Ego Chaos 동역학은 비활성이고 NPC/보행자는 별도 transient proxy로 표시 | 외부 물리 엔티티 공통 기반 |
...
| ID | 우선순위 | 상태 | 기능 | R1 완료 기준 | 향후 재사용 |
|---|---|---|---|---|---|
| TRA-001 | Must | 구현 중 | 신호 상태기계 | v1 호환을 보존하고 `signal_city_v2`의 2 controllers·차량8+보행8 heads를 같은 authoritative clock/reset으로 발행한다. 자동 검증은 통과, 실제 PIE/AT-05는 후속 | controller plan을 추가 교차로에도 재사용 |
| TRA-002 | Must | 구현 중 | TrafficDirector | 고정 route에서 차선·목표 속도·차량/보행 신호 정지 의도를 계산한다. 일반 경로 탐색·traffic demand는 후속 | C++ 물리와 분리된 rule AI |
| TRA-003 | Must | 구현 중 | NPC 차량 3~4대 | `signal_city_v2` NPC4대가 route·차량 신호·선행 장애물·lifecycle을 따르고 UE가 표시한다. 2,280-state smoke 통과, PIE 인수는 후속 | 자율주행 상호작용 대상 |
| TRA-004 | Must | 구현 중 | 보행자 6~8명 | 보행자8명이 지정 횡단보도와 보행 신호를 따르며 C++ capsule/UE 표시를 공유한다. 자동 smoke 통과, PIE 인수는 후속 | 센서와 위험 시나리오 대상 |
| TRA-005 | Must | 구현 중 | 동적 엔티티 물리 상태 | NPC4·보행자8의 authoritative identity/shape/velocity/collision과 UE 표시를 통합했다. Ego 충돌 반작용은 18-tick hold와 감쇠 상태로 다음 follower tick에도 유지되고 reset/reload에서 지워진다. SafeStop/reset/충돌 자동 검증은 통과, 실제 PIE 일치는 후속 | C++/Unreal 불일치 제거 |
| TRA-006 | Future | 연기 | 군중 시뮬레이션 | 대규모 보행자 회피·밀도 모델 | R1 제외 |
| TRA-007 | Could | 구현 중 | 사고·파손 | impulse 기반 누적 damage·impact zone을 전/후/좌/우/roof/underbody 6개 구역의 최대 12cm Unreal WPO dent로 누적한다. 새 Play/reset에서 복원되며 collision shell 변형·파편·부품 분리·fracture는 Future | 상세 crash 확장 |

### 4.6 센서와 기록

| ID | 우선순위 | 상태 | 기능 | R1 완료 기준 | 향후 재사용 |
|---|---|---|---|---|---|
| SEN-001 | Must | 구현 중 | SensorRig와 장착 좌표 | config에 `base_link` FLU front camera10Hz·roof LiDAR20Hz mount를 정의. 실제 payload 캡처는 없음 | 카메라·LiDAR·Radar 공통 |
| SEN-002 | Must | 구현 중 | 센서 시간·좌표 메타데이터 | authoritative `SimulationTimeNs`, frame/parent/pose/sequence metadata cadence 골격 구현 | 다중 센서 동기화 |
...
| REC-001 | Must | 구현 중 | 주행 기록 | `F5`가 authoritative snapshot CSV를 `Saved/DriveReplays/last_drive.csv`에 저장. command/event 전체 기록은 후속 | 회귀 시험과 데이터셋 |
| REC-002 | Must | 구현 중 | 결정적 재생 | `F6` collision-free visual ghost는 구현했지만 command/event physics re-simulation과 NFR-007 오차 검증은 미완료 | 버그 재현과 영상 촬영 |
...
최신 자동 통합은 CTest20/20·UE47/47·Signal City 2,280-state smoke까지 통과했다. 그러나
`F5/F6` snapshot/ghost는 입력 재시뮬레이션이 아니므로 AT-07/REC-002 완료 증거가 아니며,
AT-02/04/05/08의 조작감·HUD·traffic PIE·60fps·30분 시험도 미완료다.

현재 AT-01의 strict manifest·실제 collision checksum·reset 전 control 차단과 실행 중
