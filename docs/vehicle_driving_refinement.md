...
## 2026-09-08 바퀴와 차체 사이 구조 표시

바퀴가 차체에서 떨어져 보이던 공간에 차종별 연결 구조를 추가했다.
승용차·경차는 프레임과 컨트롤암·댐퍼, 트럭은 프레임·차축·댐퍼,
오토바이는 앞 포크와 뒤 스윙암·댐퍼를 표시한다. 연결 끝점은 현재 바퀴 중심을
따라가므로 기존 서스펜션의 상하 움직임에도 차체와 연결된 상태를 유지한다.

이 구조는 기존 계산 결과를 보여 주는 축약 메시다. 새 실차 서스펜션 물리나
부품별 강체·관절·충돌을 추가한 것은 아니며, 표시용 메시의 충돌은 꺼져 있다.
기존 차량·맵 자산을 다시 생성할 필요는 없다.

자동검증 결과는 [9/8 작업일지](./worklogs/2026-09-08.md)에 기록한다.
Play에서는 네 차종의 바퀴 연결부와 연석을 지날 때 연결 끝점이 끊어지지 않는지 확인한다.
