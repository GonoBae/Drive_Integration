...
9월 7일 후속 정리는 이 문서 마지막 절에 기록한다. 아래 9월 2일 검증 결과와 구분한다.
...
최근 추가한 차종·충돌·검증 코드의 중복과 의존성을 정리했다. 신규 기능이나 물리 튜닝은
포함하지 않으며, 직전 오토바이 문·NPC 앞바퀴·운전자 수정도 그대로 유지한다.

| 공통 코드 | 책임과 사용처 |
|---|---|
| `SimCoreVehicleVisualProfile` | 네 차종의 바퀴 위치·크기·표시 여부, 방향지시등·배기 위치와 NPC 실내 변환. 플레이어·NPC·운전자 표시가 같은 정의를 사용한다. |
| `RuntimeCollisionReaction` | NPC와 보행자의 충돌 운동량·정차·주행 복귀·손상 표시 상태. 도로·지면 판단과 tick 순서는 `SimulationHost`에 남긴다. |
| `physics/vehicle_parameters` | 차량 설정의 기본값과 유효성 검사. 설정 로더가 물리 적분기 전체 헤더를 포함하지 않도록 분리한다. |
| `scripts/smoke_host.py` | 통신 검사 5종의 자식 서버 실행·접속 재시도·종료. 각 검사의 nonce 확인과 성공 조건은 기존 검사에 남긴다. |

Unreal의 UCLASS/UPROPERTY·subobject 이름·키 설정·에셋 경로, 서버 기본값·물리 계산식·
메시지 필드·실행 순서·검증 거부 조건을 유지한다. 동결된 Python relay는 수정하지 않았다.

요청에 따라 루트 `CLAUDE.md`와 사용하지 않는 도구 전용 ignore 항목을 삭제했다.
대화체·작업자 중심 주석과 문서 표현은 기술 설명으로 바꿨다. NPC 판단의 AI 용어,
자동생성 파일 표식, 에셋 라이선스·출처, 과거 검증·계획 가정은 삭제하지 않았다.
삭제한 문서는 Git 이력에서 복구할 수 있으며 이력을 다시 쓰지는 않았다.

도구 단위검사 57개와 PowerShell 실행기·패키지 구성 검사를 통과했다.
서버를 중단하지 않는 관련 C++ 검사 4개와 별도 Game Development 빌드도 통과했다.
Editor 종료 후 전체 통합검증도 `automatic_pass`로 완료했다. CTest 35/35, UE Automation
94/94, 도구 검사 57/57과 실제 서버 통신·물리 재생·저장 맵 검사를 통과했다.
실제 화면·운전 감각·패키지 성능·30분 주행 인수는 별도로 남아 있다.
전체 빌드·실행 검증의 최신 결과는 [9/7 작업일지](./worklogs/2026-09-07.md)에 기록한다.

### 차로 합류·사고 보행자 후속 정리

기능 수정과 맞닿은 코드만 정리했다. 다른 물리·UI 모듈을 다시 분리하지 않는다.

- 도로: 곡선 누적 거리 계산을 `BuildArcStations`, 폭 변화 계산을 `CollectorTaperAt`으로
  모았다. 가장자리 선과 새 합류선이 같은 기준을 사용한다. 저장 맵의 이전 버전 선택도
  중첩 조건식 대신 버전별 분기로 읽을 수 있게 했다.
- NPC: `NpcRouteObstacle`에 전방 장애물 거리와 쓰러진 보행자 ID를 함께 담는다.
  `lane_npc_obstacle`을 우회·경적 판단의 공통 관측으로 사용하며, 거리만 필요한 기존
  호출은 `lane_npc_blocked_distance` 래퍼를 유지한다.
- 경적: 반복 제한은 host에 타이머를 추가하지 않고 기존 `NpcHornPolicy`에서 담당한다.
  세션 초기화, 쿨다운과 새로운 충돌 위험 경고의 계약은 정책 검사로 확인한다.

차선·충돌 확인과 후진·우회 실행은 계속 서버 책임이다. 차선 도색은 Unreal 저작 코드가
담당하며 새로운 메시지 필드나 외부 물리 SDK는 추가하지 않았다. 검증 수치는 같은 날
작업일지의 「차로 합류 표시와 사고 보행자 우회」 절에 구분해 기록한다.

## 2026-09-08 선택 리팩터링

최근 수정이 집중된 세 부분의 구조만 정리했다. NPC 판단 정책 개선, 차량 물리 튜닝,
운전자 애니메이션 변경이나 새로운 기능 추가는 포함하지 않는다. 작업 전 미커밋 변경은
유지하고, 이번 변경과 비교할 소스 사본을 별도로 보관했다.

### NPC 회피 상태

`SimulationHost::clear_npc_avoidance_state`가 회피 단계, 남은 후진 거리·속도·누적 거리,
정지 기준점과 대기 이유의 여섯 필드를 정리한다. 목적지 선택, 차선 변경 시작, 국소 우회
시작과 장애물 해소의 다섯 중복 구간에서 호출한다.

검색 차례·후보 커서·쿨다운·장애물 대기 시간·선택된 경로는 공통 초기화에 포함하지 않는다.
기존 `commit=false` 조회 경계와 계획 채택 순서를 유지한다. 우회 완료 검사에는 회피 단계와
남은 후진 거리도 초기화되는지 확인하는 조건 두 개를 보강했다.

### 플레이어와 NPC의 차종 적용

`SimCoreVehicleVisualApplication`은 기존 `SimCoreVehicleVisualProfile`을 대체하지 않는다.
프로필은 차종별 값의 원본으로 남고, 새 내부 함수는 반복된 적용 절차만 담당한다.

- `SelectBody`: 미지정 차종을 세단으로 정규화하고 네 차종의 차체 메시를 선택한다.
- `ReplaceBodyMesh`: 재질 덮어쓰기 해제 → 메시 교체 → 로컬 변환 초기화 → 표시 순서를 유지한다.
- `ConfigureAttachments`: 방향지시등 → 운전자 → 서스펜션 순서로 기존 프로필을 적용한다.

플레이어의 바퀴 피벗과 NPC의 바퀴 직접 변환은 각 Actor에 남긴다. 플레이어 무게중심,
NPC 충돌 중심, 재생 원점, 차체 bounds, 경적 위치, 손상 초기화와 조기 반환 순서도 유지한다.
UCLASS·UPROPERTY·기본 subobject 이름·입력·에셋 경로는 변경하지 않는다. 새 Automation은
차종 정규화, 미지원 차종·누락 메시 거부와 메시 교체 시 변환·재질·표시 상태를 검사한다.

### 차량 물리 업데이트 경계

물리 업데이트에서 다음 단계들을 private 함수로 분리했다.

| 단계 | 책임 |
|---|---|
| `capture_supported_pose` | 틱 시작의 기존 15개 상태·서스펜션 값을 하나의 스냅샷으로 저장 |
| `advance_drive_force` | 기존 구동력 응답 계산과 구동 억제 여부 반환 |
| `advance_steering` | 기존 조향 랙 속도 제한과 표시 조향각 갱신 |
| `has_supported_ground_coverage` | 차체 중심과 차축·좌우 영역의 기존 지면 범위 판정 |
| `restore_supported_pose_and_stop` | 범위 밖 시 이전 자세·서스펜션 복원, 운동 정지 및 시도한 반력 제거 |
| `update_powertrain_telemetry` | 승인된 틱의 RPM·기어·연료 갱신 |
| `commit_wheel_support_feedback` | 승인된 틱의 압축 한계 반력을 다음 틱의 힘 추정으로 확정 |

지면 범위 판정은 바퀴의 하중·서스펜션 접촉 판정과 다르다. 복구도 전체 상태를 무조건
되감는 동작이 아니다. 기존처럼 속도와 반력 이력은 지우되 현재 틱의 조향 요청은 유지한다.
실패한 틱은 RPM·연료 및 충돌 피드백을 확정하기 전에 반환한다. 해당 경계 검사에는 연료·손상
유지, idle RPM, 외부 접촉·바퀴 반력 이력 삭제 조건을 추가했다.

수식, 상수, 부동소수 자료형과 연산 순서는 유지했다. 서로 강하게 연결된 타이어 힘·충돌·
자세 적분의 핵심 구간을 한꺼번에 재설계하지는 않았다. 이번 정리의 목적은 상태 저장·복구와
승인 경계를 명확히 하는 것이며, 전체 물리 모듈을 완전히 분리했다는 뜻은 아니다.

검증 결과와 실행 중인 서버의 반영 여부는 [9/8 작업일지](./worklogs/2026-09-08.md)의
「선택 리팩터링 검증」 절에 구분해 기록한다.
