...
| traffic checksum | 현재 서버 시작 로그의 traffic checksum |
...
북쪽 연결 루프, 독립된 보호 좌회전 신호를 추가했다. checksum은 현재 manifest와 서버
로그를 확인하고, 검증 실적은 [현재 상태](../status.md)를 참고한다.
...
재Bake 뒤에는 manifest와 두 commandlet의 성공 로그를 확인하고 검증 상태를 갱신한다.
traffic JSON의 `source_map_checksum`은 manifest의 collision checksum과
...
- server-authoritative NPC 10대는 `npc_autonomous=true`에서 목적지를 선택하고 합법적인
  ...
  신호를 유지한다. [NPC 내비게이션 범위·안전 계약](../reference/npc_navigation.md)을 따른다.
...
[차선·신호 문서](../reference/traffic_network_signals.md#traffic-network-version-2와-다중-controller)를 따른다.
...
[충돌·파손의 구현 경계](../reference/collisions.md)를 함께 따른다.
...
사고 차량과 쓰러진 보행자에게는 경적을 울리지 않는다. 안전한 우회·후진을 검토하고,
통과할 공간이 없으면 대기한다.
