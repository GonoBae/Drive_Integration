...
- 이 문서는 차선·신호의 구현 계약과 검증 절차를 기록한다. 현재 시험 결과와 수동 인수 상태는 [검증 현황](../status.md), 개발 당시 기록은 [이전 자료](../README.md#이전-자료)에서 확인한다.
...
- `signal_city_v2`는 별도 맵·MapPackage와 traffic format version 2를 사용해 현재 59개 lane,
  ...
  바꾸지 않는다. 실행 순서는 [signal city 빠른 시작](../guides/signal_city_quickstart.md)을 따른다.
- `drive_route.csv`는 차량 물리 완주를 확인하는 QA 경로다. 이번 `traffic_network.json`은 별도로 작성한 차선·교차로 연결 데이터이며 두 파일을 같은 기능으로 취급하지 않는다.
- v1 NPC 1대 기반을 보존하고 Signal City는 NPC10대·보행자8명으로 확장했다. NPC 목적지 선택·우회·차선 변경의 현재 범위는 [NPC 내비게이션](./npc_navigation.md)을 따른다. 최초 단일 NPC 이력은 [이전 자료](../README.md#이전-자료)에 보관한다. 최종 사용자 인수·전체 TrafficDirector 완료를 의미하지 않는다.
...
생성 수치는 이력이며 현재 생성물은 [Signal City 안내](../guides/signal_city_quickstart.md),
시험 결과는 [검증 현황](../status.md)을 따른다.
...
신호 actor는 `NoCollision`이다. 표시 추가가 기존 ground/static collider checksum이나 차량 충돌 형상을 바꾸지 않는다. 신호 actor는 게임 world에서 수신 상태에 따라 생성되는 표시용 객체이며, 저장 맵에 새 영구 충돌체를 배치하는 기능이 아니다. `virtual_city_v1`의 정지선은 lane 끝의 제어 metadata이며, 본선의 새 정지선 페인트는 아직 추가하지 않았다. 기존 맵의 지선 정지선 표시는 그대로 보존했다.

## Virtual City v1 저작 topology
...
않는다. 맵·생성·실행의 전체 순서는 [signal city 빠른 시작](../guides/signal_city_quickstart.md)을
...
[signal city 빠른 시작](../guides/signal_city_quickstart.md#빌드와-생성-순서)에 따로 적는다.
...
아래는 실행 방법/확인 항목이다. **명령이 존재하거나 코드가 작성되었다는 사실만으로 통과 처리하지 않는다.** 현재 결과·인수 상태는 [검증 현황](../status.md)에 기록한다.
...
- Python `test_smoke_traffic.py`: 다른 서버 nonce/누락 capability에는 제어 프레임을 보내지
  않는 방어, v1/v2 expected JSON/phase와 원자 WorldState helper를 검사한다.
  9/2 당시 7/7이 통과했다. 이 단위 검사는 서버 프로세스나 소켓을 생성하지 않는다.
- 실제 `smoke_traffic.py`: 랜덤 localhost port의 **자체 생성 child**만 사용한다. 사용자 기본
  ...
  controller/group plan과 offset을 직접 읽어 검증한다. 9/2 당시 `signal_city_v2` 실행은 38초,
  ...
  자신의 child만 종료한다.
- 9/2 당시 v2 근거는 `runtime_logs/traffic-smoke-20260902-110742-ed34c190.*.log`다. 시작 진단은
  controller 1·2를 각각 `cycle_seconds=36`, `offset_seconds=0/9`로 출력했다. 같은 당시
...
위 9/2 결과는 초기 v2의 이력이며 현재 신호 구성의 검증 결과를 대신하지 않는다.
과거 로그 경로의 보관 위치는 [이전 자료](../README.md#이전-자료)에서 확인한다.
...
[signal_city_v2 사용자 PIE 인수](../guides/signal_city_quickstart.md#사용자-pie-인수)를 별도로 수행한다.
...
Signal City의 NPC·보행 신호 확장은 현재 구현에 포함된다. 실제 신호 가독성·NPC 통과·
주행 자연스러움과 패키지 성능·30분 안정성은 [검증 현황](../status.md)의 순서대로 확인한다.
