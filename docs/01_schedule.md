...
| 현재 관리 기준 | 기존 Core 9/7·확장 9/11 목표는 미인수. 안정화→실측→배포 인수 순서로 진행하며 새 완료일은 미확정 |
| 개발 인원 | 1명 |
| 기준 작업량 | 1일 8시간 |
...
59개 차로·NPC10대·보행자8명과 플레이어4차종이 현재 구현 범위다.
이번 순서는 입력 누적 지연 → NPC 우회 중 정체 복구 → 준비 비용을 포함한 탐색 계측 →
입력 반응 측정 절차 → 최신 배포 후보 → 문서 동기화다. 결과와 잔여 조건은
[9/17 작업 기록](./worklogs/2026-09-17.md)을 기준으로 한다.

실제 패키지의 화면·청감, 목표 PC 성능·입력 반응·30분 연속 주행은 별도 인수다.
자동 검사나 패키지 생성만으로 이 조건을 완료 처리하지 않는다. 새로운 기능 확장일은
이 인수 결과를 확인한 다음 정한다. 아래 9/7 설명과 날짜별 표는 당시 계획·실적 이력이다.
...
| 버전 | 날짜 | 변경 내용 |
|---|---|---|
| 2.25 | 2026-09-17 | 입력 누적 지연·우회 중 재차 차단 복구·탐색 준비 비용 계측과 실측 절차·배포 후보 식별을 반영. CTest 최초47+수정 후1, UE114·도구102 및 패키지 경로 이동·연결 검사 통과. 기존 목표일은 이력으로 구분하며 화면·입력 실측·30분 주행 인수와 새 완료일은 미확정 |
